<?= $this->extend('layout/main') ?>

<?= $this->section('content') ?>

<div class="section-body">
    <div class="row">
        <div class="col-10">
            <div class="card">
                <div class="card-header">
                    <div style="text-align: center;">
                        <img src="<?= base_url(); ?>public/assets/images/logo/kantina.png" alt="Logo" height="100px" />
                        <h6 class="mt-2">Manajementu Kantina Eskola</h6>
                    </div>
                    <h4 class="text-primary mt-3">Detallu Order</h4>
                </div>

                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-borderless">
                            <tbody>

                                <tr>
                                    <th width="20%" class="text-dark fw-bold">
                                        <i class="fas fa-user"></i> Naran Kostumer
                                    </th>
                                    <td><?= $order['naran'] ?></td>
                                </tr>

                                <tr>
                                    <th class="text-dark fw-bold">
                                        <i class="fas fa-box"></i> Produtu
                                    </th>
                                    <td><?= $order['naran_produtu'] ?></td>
                                </tr>

                                <tr>
                                    <th class="text-dark fw-bold">
                                        <i class="fas fa-tag"></i> Presu ($)
                                    </th>
                                    <td><?= $order['presu'] ?></td>
                                </tr>

                                <tr>
                                    <th class="text-dark fw-bold">
                                        <i class="fas fa-sort-numeric-up"></i> Kuantidade
                                    </th>
                                    <td><?= $order['kuantidade'] ?></td>
                                </tr>

                                <tr>
                                    <th class="text-dark fw-bold">
                                        <i class="fas fa-dollar-sign"></i> Total Presu ($)
                                    </th>
                                    <td>
                                        <?= number_format($order['presu'] * $order['kuantidade'], 2) ?>
                                    </td>
                                </tr>

                            </tbody>
                        </table>
                    </div>
                </div>

            </div>
        </div>
    </div>
</div>

<?= $this->endSection() ?>